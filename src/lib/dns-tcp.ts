...
export function patchDnsLookup(): void {
  ...
  const patchedLookup = function (
    ...
    resolveViaTcp(hostname, family === 6 ? 28 : family === 4 ? 1 : 0)
      .then((results) => {
        if (all) {
          ...
          const pick = family === 6 ? v6 || v4 : v4 || v6
          if (pick) {
  ...
  ;(dns as { lookup: typeof patchedLookup }).lookup = patchedLookup

  async function resolveViaTcp(
    hostname: string,
    type: number
  ): Promise<{ address: string; family: number }[]> {
    const servers = ["10.152.183.213", "8.8.8.8", "1.1.1.1"]
    try {
      const result = await queryWithFallback(servers, hostname, type)
      return result.answers.map((a) => ({
        address: a.data,
        family: a.type === 28 ? 6 : 4,
      }))
    } catch {
      try {
        const addrs = await new Promise<dns.LookupAddress[]>((resolve, reject) => {
          originalLookup(hostname, { family: type === 28 ? 6 : 4, all: true }, (err, addrs) => {
            if (err) reject(err)
            else resolve(Array.isArray(addrs) ? addrs : [{ address: addrs, family: type === 28 ? 6 : 4 }])
          })
        })
        return addrs
      } catch {
